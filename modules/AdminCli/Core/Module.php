...
return new class implements ModuleContract {
    ...
    {
        return 'AdminCli/Core';
    }

    public function register(): void
    {
        $path = __DIR__ . '/Config/' . strtolower('AdminCli') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('AdminCli'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('AdminCli')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('AdminCli'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('AdminCli')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('AdminCli'), __DIR__ . '/views');
