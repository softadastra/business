...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Chat') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Chat'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Chat')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Chat'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Chat')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Chat'), __DIR__ . '/views');
