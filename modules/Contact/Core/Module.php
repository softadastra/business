...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Contact') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Contact'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Contact')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Contact'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Contact')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Contact'), __DIR__ . '/views');
