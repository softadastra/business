...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Landing') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Landing'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Landing')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Landing'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Landing')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Landing'), __DIR__ . '/views');
