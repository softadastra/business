...
return new class implements ModuleContract {
    ...
    public function register(): void
    {
        ...
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Order'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Order')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Order'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Order')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Order'), __DIR__ . '/views');
