...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Payments') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Payments'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Payments')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Payments'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Payments')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Payments'), __DIR__ . '/views');
