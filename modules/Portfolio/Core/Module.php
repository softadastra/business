...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Portfolio') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Portfolio'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Portfolio')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Portfolio'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Portfolio')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Portfolio'), __DIR__ . '/views');
