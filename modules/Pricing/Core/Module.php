...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Pricing') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Pricing'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Pricing')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Pricing'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Pricing')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Pricing'), __DIR__ . '/views');
