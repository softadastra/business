...
return new class implements ModuleContract {
    ...
    {
        return 'Princing/Core';
    }

    public function register(): void
    {
        $path = __DIR__ . '/Config/' . strtolower('Princing') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Princing'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Princing')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Princing'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Princing')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Princing'), __DIR__ . '/views');
