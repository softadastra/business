...
return new class implements ModuleContract {
    ...
    {
        return 'Testimonials/Core';
    }

    public function register(): void
    {
        $path = __DIR__ . '/Config/' . strtolower('Testimonials') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Testimonials'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Testimonials')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Testimonials'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Testimonials')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Testimonials'), __DIR__ . '/views');
